...
batchAddVariable({ globalParamList: validList })
    .then(res => {
        if (res.code === SERVER_ERROR_CODE.SUCCESS) { // !!!Bad: how many interface, how many judge 200
            this.close(true)
            ...
            if (res.code === SERVER_ERROR_CODE.REPEAT) {
                ...
            }
        }
    })
...
多么简洁的代码，但Promise执行reject代码，浏览器会报`Uncaught (in promise)`错误。这是因为中断了Promise操作但又没有对其进行处理，故有此错误。只需要使用`unhandledrejection`全局处理即可。
